...
if (isset($_POST)) {
  // print_r_html($_POST);
  if (
    empty(trim($_POST['item_code'])) || empty(trim($_POST['item_name'])) || empty(trim($_POST['item_category']))  || empty(trim($_POST['unit'])) || empty(trim($_POST['tire_size'])) || empty(trim($_POST['tire_design'])) || empty(trim($_POST['rim_size']) || empty(trim($_POST['load_index'])) || empty(trim($_POST['load_index'])) || empty(trim($_POST['speed_rating'])) || empty(trim($_POST['ply_rating'])) || empty(trim($_POST['type'])))
  ) {
    ...
    $sql = "INSERT INTO tb_items (`sku_code`, `material_description`,  `category`, `unit`,`tire_size`, `tire_design`, `rim_size`,`load_index`, `speed_rating`, `ply_rating`, `type`,`created_by`) VALUES('$item_code','$item_name','$item_category','$unit','$tire_size','$tire_design','$rim_size','$load_index','$speed_rating','$ply_rating','$type','$created_by')";
